...
interface DotsBackdropProps {
  className?: string;
  color?: string; // rgb string like "87, 220, 205"
  opacityMin?: number;
  opacityMax?: number;
  dotSpacing?: number;
  baseRadius?: number;
...
export const DotsBackdrop: React.FC<DotsBackdropProps> = ({
  className = "",
  color = "87, 220, 205",
  opacityMin = 0.4,
  opacityMax = 0.5,
    ...
    for (let i = 0; i < cols; i++) {
      for (let j = 0; j < rows; j++) {
        ...
        newDots.push({
          x,
          y,
          baseColor: `rgba(${color}, ${opacityMax})`,
  ...
  }, [GRID_CELL_SIZE, color, dotSpacing, opacityMax, opacityMin, baseRadius]);
  ...
  const animateDots = useCallback(() => {
    ...
    animationFrameId.current = requestAnimationFrame(animateDots);
  }, [GRID_CELL_SIZE, INTERACTION_RADIUS_SQ, interactionRadius, opacityBoost, radiusBoost, opacityMin, opacityMax]);
